-- Migration 014: Pattern report history & alert log
--
-- Persists the daily pattern-insights snapshot so AI agents and sensitive
-- automation workflows can query the latest win/loss signatures without
-- needing the local filesystem cache.
...
COMMENT ON TABLE pattern_cache_alerts IS
    'Alert history emitted after each report refresh (used by automation/webhooks)';
