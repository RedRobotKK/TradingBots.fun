...
impl SignalWatchlist {
    ...
    pub async fn re_evaluate(
        ...
        if entry.status == WatchStatus::Expired {
            return None;
        }
        ...
        let direction_flipped = current_action != "SKIP" && current_action != entry.action;
        ...
        if direction_flipped || age_exceeded || price_missed {
            let reason = if direction_flipped {
                ...
                let drift = (current_price - entry.original_price) / entry.original_price * 100.0;
        ...
        let same_direction = current_action == entry.action || current_action == "SKIP"; // SKIP = not contradicting

        let strengthened =
            same_direction && current_confidence >= entry.original_confidence + STRENGTHEN_DELTA;
...
mod tests {
    ...
    fn watch_threshold_filters_low_confidence() {
        ...
        rt.block_on(async {
            ...
            wl.maybe_watch("ETH", "BUY", 0.35, 3000.0, SkipReason::LowConfidence)
                .await;
    ...
    fn watch_above_threshold_is_added() {
        ...
        rt.block_on(async {
            let wl = SignalWatchlist::new();
            wl.maybe_watch("ETH", "BUY", 0.55, 3000.0, SkipReason::LowConfidence)
                .await;
    ...
    fn re_evaluate_still_viable() {
        ...
        rt.block_on(async {
            let wl = SignalWatchlist::new();
            wl.maybe_watch("SOL", "BUY", 0.55, 100.0, SkipReason::LowConfidence)
                .await;
    ...
    fn re_evaluate_strengthened_when_confidence_jumps() {
        ...
        rt.block_on(async {
            let wl = SignalWatchlist::new();
            wl.maybe_watch("BTC", "BUY", 0.55, 50000.0, SkipReason::LowConfidence)
                .await;
    ...
    fn re_evaluate_expires_on_direction_flip() {
        ...
        rt.block_on(async {
            let wl = SignalWatchlist::new();
            wl.maybe_watch("BTC", "BUY", 0.60, 50000.0, SkipReason::LowConfidence)
                .await;
