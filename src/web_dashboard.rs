...
function resetStats(){{
  ...
  fetch('/api/admin/reset-stats',{{method:'POST',credentials:'include'}})
    ...
    .then(function(d){{
      resp.style.display='block';
      if(d.ok){{resp.style.background='var(--green-dim)';resp.style.color='var(--green)';resp.style.border='1px solid #22c55e44';resp.textContent='✅ '+d.message;btn.textContent='✓ Done';_toast('Stats reset','#22c55e');setTimeout(function(){{window.location.reload();}},2000);}}
