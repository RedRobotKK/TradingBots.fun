...
impl HyperliquidClient {
    ...
    pub async fn get_account(&self) -> Result<Account> {
        ...
        let state: ClearinghouseState = resp
            .json()
            .await
    ...
    pub async fn place_order(
        ...
        let action = HlAction {
            ...
            builder: self
                .builder_code
                .clone()
                .map(|b| HlBuilder { b, f: clamped_bps }),
        };
...
fn sign_l1_action(action: &HlAction, nonce: u64, testnet: bool, key: &[u8]) -> Result<HlSignature> {
    ...
    .into();
    let agent_type_hash: [u8; 32] =
        Keccak256::digest(b"Agent(string source,bytes32 connectionId)").into();
...
fn symbol_to_asset_index(symbol: &str) -> Result<u32> {
    ...
    let idx = match sym {
        ...
        _ => {
            return Err(anyhow!(
                "Unknown HL asset index for '{}'. Add it to symbol_to_asset_index() in exchange.rs",
                sym
            ))
        }
    };
...
mod tests {
    ...
    fn builder_serialises_with_correct_field_names() {
        ...
        assert_eq!(
            j["b"], "0xa765cd52ad56efc294fc6a7155a53920294ca3e3",
            "builder address field must be 'b'"
        );
        assert_eq!(j["f"], 3, "fee field must be 'f'");
    ...
    fn fee_bps_below_max_passes_through_unchanged() {
        for input in [0u32, 1, 2, 3] {
            ...
            assert_eq!(
                clamped, input,
                "fee_bps {} should not be altered, got {}",
                input, clamped
            );
        }
    }
    ...
    fn builder_fee_never_exceeds_hl_maximum() {
        ...
        for tier in [TenantTier::Free, TenantTier::Pro, TenantTier::Internal] {
            ...
            cfg.tier = tier.clone();
            assert!(
                cfg.builder_fee_bps() <= 3,
                "{:?} tier fee {} exceeds HL maximum of 3 bps",
                tier,
                cfg.builder_fee_bps()
            );
        }
    ...
    fn btc_and_eth_have_correct_asset_indices() {
        ...
        assert_eq!(
            symbol_to_asset_index("BTCUSDT").unwrap(),
            0,
