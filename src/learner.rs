...
fn default_ema_cross_weight() -> f64 {
    0.07
}
fn default_z_score_weight() -> f64 {
    0.17
}
...
fn default_funding_rate_weight() -> f64 {
    0.08
}
fn default_candle_pattern_weight() -> f64 {
    0.04
}
fn default_chart_pattern_weight() -> f64 {
    0.04
}
...
impl SignalWeights {
    ...
    pub fn update(&mut self, contrib: &SignalContribution, was_long: bool, profitable: bool) {
        ...
        if contrib.candle_pattern_present {
            nudge(
                &mut self.candle_pattern,
                contrib.candle_pattern_bullish,
                was_long,
                profitable,
                win_scale,
                lose_scale,
        ...
        if contrib.chart_pattern_present {
            nudge(
                &mut self.chart_pattern,
                contrib.chart_pattern_bullish,
                was_long,
                profitable,
                win_scale,
                lose_scale,
...
mod tests {
    ...
    fn all_bullish_contrib() -> SignalContribution {
        SignalContribution {
            ...
            trend_bullish: true,
            of_bullish: true,
            z_score_present: true,
            z_score_bullish: true,
            volume_present: true,
            volume_bullish: true,
            ...
            candle_pattern_present: false,
            candle_pattern_bullish: false,
            chart_pattern_present: false,
    ...
    fn default_weights_sum_to_one() {
        ...
        assert!(
            (total - 1.0).abs() < 1e-6,
            "default weights sum={total:.8} ≠ 1.0"
        );
    }
    ...
    fn signal_contribution_default_all_false() {
        ...
        assert!(
            !c.candle_pattern_present,
            "new pattern field must default to false"
        );
        assert!(
            !c.chart_pattern_present,
            "new pattern field must default to false"
        );
    }
    ...
    fn signal_contribution_old_json_without_new_fields_deserialises_ok() {
        ...
        assert_eq!(
            c.entry_confidence, 0.0,
            "old JSON should give 0.0 confidence"
        );
    }
    ...
    fn loss_penalty_exceeds_win_reward_so_bad_signals_decay() {
        ...
        for _ in 0..4 {
            w.update(&contrib, true, false);
        }
        // 5 wins, 4 losses = 55.6% win rate — weight should be lower than start
        assert!(
            w.rsi < start,
            "55% win rate should not maintain weight: start={start:.4} end={:.4}",
            w.rsi
        );
    }
